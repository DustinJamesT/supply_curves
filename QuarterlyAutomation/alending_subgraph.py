...
class alending_subgraph(subgraph):
    ...
    ###############
    # Snapshots
    async def get_market_snapshots(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000, refresh: bool=False) -> pd.DataFrame:
        
        if  not self.market_snapshots.empty and not refresh:
            return self.market_snapshots
        
        # input handling
        stop_timestamp = end_timestamp if end_timestamp else int(datetime.datetime.now().timestamp())
        ...
        inputTokens_df.columns = [f'inputTokens.{col}' for col in inputTokens_df.columns]
        ...
        outputToken_df.columns = [f'outputToken.{col}' for col in outputToken_df.columns]
    ...
    async def get_financial_snapshots(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000, refresh: bool=False) -> pd.DataFrame:
    ...
    async def get_snapshots(self, start_timestamp: int=0, end_timestamp: int=None, first: int=1000, refresh: bool=False) -> pd.DataFrame:
    ...
    def clean_events_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        tmp_df['amount'] = tmp_df['amount'].astype(float) / (10**tmp_df['asset.decimals'])
        tmp_df['network'] = tmp_df['protocol'].apply(lambda x: x['network']) # NOTE: must go before protocol
        ...
        # Decimal adjusting
        tmp_df['amount'] = tmp_df['amount'] / (10 ** tmp_df['asset.decimals'].astype(int))
    ...
    ###############
    # Events
    async def get_deposits(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000) -> pd.DataFrame:
    ...
    async def get_withdraws(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000) -> pd.DataFrame:
    ...
    async def get_borrows(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000) -> pd.DataFrame:
    ...
    async def get_repays(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000) -> pd.DataFrame:
    ...
    async def get_liquidates(self, session, start_timestamp: int=0, end_timestamp: int=None, first: int=1000) -> pd.DataFrame:
    ...
    async def get_events(self, start_timestamp: int=0, end_timestamp: int=None, first: int=1000, refresh: bool=False) -> pd.DataFrame:
        # Checking for cache
    ...
    def label_users(self, events_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        user_actions = events_df.groupby(['user', 'action'])['amountUSD'].sum().unstack()
        user_actions['volume'] = user_actions[['deposit','borrow']].fillna(0).sum(axis=1)
        user_actions.columns = [f'{col}.usd' for col in user_actions.columns]

        # Has this address ever borrowed
        user_actions['borrower'] = user_actions['borrow.usd'].apply(lambda x: True if x > 0 else False)

        # Note: This is people who liquidate others, not those who get liquidated
        # TODO: We need to see when people get liquidated
        user_actions['liquidator'] = user_actions['liquidate.usd'].apply(lambda x: True if x > 0 else False)

        # Has this address ever withdrawed
        user_actions['saver'] = user_actions['withdraw.usd'].fillna(0).apply(lambda x: True if x == 0 else False)

        # Whale is volume over 1mil
        user_actions['whale'] = user_actions['volume.usd'].apply(lambda x: True if x > 1000000 else False)

        # Get top ten users
        top_ten_vol = user_actions['volume.usd'].sort_values(ascending=False)[:10][-1]
        user_actions['top.ten'] = user_actions['volume.usd'].apply(lambda x: True if x >= top_ten_vol else False)
        ...
        user_firsts['first.dt'] = user_firsts.min(axis=1) # must do this to get first time using protocol

        # NOTE: had to disable for sreamlit, will re-enable later
        user_firsts['time-to-borrow'] = user_firsts['borrow.first.dt'] - user_firsts['deposit.first.dt']

        # Get how big users first actions were
        user_first_amounts = events_df.groupby(['user', 'action'])['amount'].first().unstack()
        user_first_amounts.columns = [f'{col}.first.usd' for col in user_first_amounts.columns]
